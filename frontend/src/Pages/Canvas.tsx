...
import { normalizePoints } from "../helpers/normalize-points.ts";
import { Stage, Layer, Image, Circle } from "react-konva";
import { useUserSession } from "../auth/globalState.tsx";

import * as tf from "@tensorflow/tfjs";
import Konva from "konva";
import { apiUploadBlob, apiUpdateCanvas } from "../api";
...
function Canvas() {
  ...
  async function startWebcam() {
    ...
    try {
      ...
      const mediastream = await navigator.mediaDevices.getUserMedia({
        video: { width: 200, height: 200, frameRate: { ideal: 60 } },
      });
  ...
  function processResult(result: any) {
    ...
    for (const handLandmark of result.landmarks) {
      ...
      overlay.drawConnectors(handLandmark, GestureRecognizer.HAND_CONNECTIONS, {
        color: "green",
      });
      xTrue =
        ((handLandmark[8].x * -1 + 1) * window.innerWidth - stage.x()) /
        oldScale;
    ...
    if (MODE === "zoom out" && landmarks && landmarksRight) {
      ...
      const distance = Math.sqrt(
        (landmarksRight[8].x - landmarksRight[4].x) ** 2 +
          (landmarksRight[8].y - landmarksRight[4].y) ** 2,
      );
      zoom = false;
    ...
    if (displayArr.includes("ZOOM OUT") && displayArr.includes("STOP"))
      MODE = "zoom out";
    if (displayArr.includes("ZOOM IN") && displayArr.includes("CLOSE"))
      MODE = "zoom in";
    ...
    if (modesRef.current) {
      Array.from(modesRef.current.children).forEach((child: any) => {
        ...
        child.style.background = active ? "#303036" : "#858585";
      });
    }
  ...
  function controlZoom(distance: number, zoom: boolean) {
    ...
    if (distanceArr.length > 1) {
      const DISTBOOL = !zoom
        ? distanceArr[1] - distanceArr[0] < THRESHOLD
        : distanceArr[1] - distanceArr[0] > THRESHOLD;
      if (DISTBOOL) {
  ...
  async function save() {
    ...
    const blobResponse = await apiUploadBlob(id!, formData, accessToken);
    const publicUrl = blobResponse.data;

    await apiUpdateCanvas(id!, publicUrl);
  ...
  return (
    ...
    >
      <Sidebar
        ...
        clearAll={clearAll}
        switchCanvas={switchOutCanvas}
        webcamActive={webcamSelected}
        loading={loading}
      />
        ...
        >
          ...
          {w > 1 && (
            ...
            >
              ...
              <Layer ref={overlayRef}></Layer>
            </Stage>
          )}
        </div>
        <div
          className="border-l border-[#222226] flex flex-col overflow-hidden"
          style={{ background: "#1a1a1e" }}
        >
          <div className="relative w-full flex-shrink-0">
            <video className="w-full scale-x-[-1]" ref={videoRef} autoPlay />
            <canvas
              className="w-full h-full absolute right-0 top-0 z-10 scale-x-[-1]"
              ref={visualRef}
            ></canvas>
          </div>
